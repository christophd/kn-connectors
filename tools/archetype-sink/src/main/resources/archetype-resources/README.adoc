...
[source,text]
----
quay.io/openshift-knative/kn-connector-${kamelet-name}-sink:1.0-SNAPSHOT
----
...
[source,shell]
----
kubectl set env deployment/kn-connector-${kamelet-name}-sink CAMEL_KAMELET_{{KAMELET_NAME}}_{{PROPERTY_NAME}}="I updated it..."
----
...
Given a configmap named `my-${kamelet-name}-sink-config` in Kubernetes that has two entries:

.my-${kamelet-name}-sink-config
[source,properties]
----
...
* CAMEL_KAMELET_{{KAMELET_NAME}}_MESSAGE={{configmap:kn-sink-config/message}}
* CAMEL_KAMELET_{{KAMELET_NAME}}_PERIOD={{configmap:kn-sink-config/period}}
...
[source,text]
----
configmap:kn-sink-config/period:5000
----
...
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: ${kamelet-name}-sink
      app.kubernetes.io/version: 1.0-SNAPSHOT
  template:
    spec:
      containers:
        - image: localhost:5001/openshift-knative/${kamelet-name}-sink:1.0-SNAPSHOT
          imagePullPolicy: Always
          name: ${kamelet-name}-sink
          env:
            - name: CAMEL_KAMELET_{{KAMELET_NAME}}_MESSAGE
              value: "{{configmap:kn-sink-config/message}}"
            - name: CAMEL_KAMELET_{{KAMELET_NAME}}_PERIOD
              value: "{{configmap:kn-sink-config/period:1000}}"
----
...
spec:
  ...
  template:
    spec:
      ...
      volumes:
        - configMap:
            name: my-${kamelet-name}-sink-config
          name: kn-sink-config
----
...
* CAMEL_KAMELET_{{KAMELET_NAME}}_MESSAGE=secret:kn-sink-config/msg
* CAMEL_KAMELET_{{KAMELET_NAME}}_PERIOD=secret:kn-sink-config/period
